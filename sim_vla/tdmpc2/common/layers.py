...
def conv(in_shape, num_channels, out_dim, act=None):
	...
	Convolutions with ReLU activations, followed by a linear layer projecting
	the flattened features to `out_dim` when they do not already match it.
	"""
	...
	if in_shape[-1] == 64:
		...
		convs = [
			...
			nn.Conv2d(num_channels, num_channels, 3, stride=2), nn.ReLU(inplace=True),
			nn.Conv2d(num_channels, num_channels, 3, stride=1), nn.Flatten()]
	layers = [ShiftAug(), PixelPreprocess(), *convs]
	with torch.no_grad():
		flat_dim = nn.Sequential(*convs)(torch.zeros(1, *in_shape)).shape[-1]
	if flat_dim != out_dim:
		layers.append(nn.Linear(flat_dim, out_dim))
	if act:
		layers.append(act)
