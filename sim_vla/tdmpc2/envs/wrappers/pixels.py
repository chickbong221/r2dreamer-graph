...
class PixelWrapper(gym.ObservationWrapper):
	...
	def __init__(self, cfg, env, num_envs, num_frames=1):
		...
		self.rgb_stack = torch.zeros((*self.rgb_shape, self.num_frames), dtype=torch.uint8, device=self.base_env.device)
		if 'state' in self.base_env._init_raw_obs:
			self.state_shape = self.base_env._init_raw_obs['state'].shape # (num_envs, n)
			self.state_stack = torch.zeros((*self.state_shape, self.num_frames), dtype=torch.float32, device=self.base_env.device)
