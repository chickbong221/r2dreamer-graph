...
class SOLDModule(OnlineModule):
    def __init__(self, autoencoder: Autoencoder, dynamics_predictor, actor, critic, reward_predictor,
        ...
        # Latent-conditioned actor hook. None is the upstream module: the
        # Gaussian actor is sampled with rsample(), its log-probabilities and
        # entropies are collected, and compute_actor_loss uses them exactly as
        # before. An attached policy only has to answer `sample(slots, start)`
        # and hold its own optimizer; nothing here knows what is behind it.
        # See sim_vla/integrations/sold/policy.py.
        self.latent_policy = None

    def attach_policy(self, policy) -> None:
        """Route actor sampling at an external, latent-conditioned policy.

        Assigned as a submodule, so Lightning's checkpointing carries its
        weights. Its optimizer is its own: `configure_optimizers` returns four
        optimizers and their order is positional, so adding a fifth would
        renumber the ones `training_step` unpacks.
        """
        if policy is not None and self.actor_gradients == "reinforce":
            raise ValueError(
                "actor_gradients='reinforce' cannot be used with a flow "
                "policy: REINFORCE is log_prob(a) * advantage, and a "
                "flow-matching policy has no tractable log-probability. The "
                "flow-matching loss is a regression onto a velocity, not a "
                "density, and substituting it here would optimise nothing. "
                "Use actor_gradients='dynamics', which differentiates the "
                "imagined return through the sampled action.")
        self.latent_policy = policy

    def _actor_step(self, slots_context: torch.Tensor, start: int):
        """One action from the actor, with whatever else that actor offers.

        Returns `(action, log_prob, entropy)`. The Gaussian actor supplies all
        three; a flow policy supplies only the action and `None` for the other
        two, and `compute_actor_loss` drops the terms that read them rather
        than being handed a stand-in for a density it does not have.
        """
        if self.latent_policy is None:
            action_dist = self.actor(slots_context.detach(), start=start)
            selected_action = action_dist.rsample().squeeze(1)
            return (selected_action,
                    action_dist.log_prob(selected_action.unsqueeze(1)),
                    action_dist.entropy())
        selected_action = self.latent_policy.sample(
            slots_context.detach(), start=start, grad=True)
    ...
    def training_step(self, batch, batch_index: int) -> STEP_OUTPUT:
        ...
        # Learn the actor. With a latent policy attached the objective is the
        # same lambda-return term under dynamics gradients; what changes is
        # which parameters it steps and that the entropy bonus is absent.
        ...
        if policy is None:
            ...
            # The same norm clipping at the same actor_grad_clip;
            # Trainer.clip_gradients only knows the optimizers
            # configure_optimizers returned, and this one is the policy's own.
            policy.step(self.actor_grad_clip)
    ...
    def imagine_ahead(self, slots: torch.Tensor, actions: torch.Tensor) -> List[torch.Tensor]:
        ...
        with FreezeParameters([self.reward_predictor, self.critic]):
            for t in range(self.imagination_horizon):
                selected_action, log_prob, entropy = self._actor_step(
                    slots_context, start=slots_context.shape[1] - 1)
        ...
        # Empty when a flow policy supplied the actions: it has neither a
        # log-probability nor an entropy, and compute_actor_loss drops the
        # terms that would have read them.
        action_log_probs = (torch.stack(action_log_probs, dim=1).squeeze(2)
                            if action_log_probs else None)
        action_entropies = (torch.stack(action_entropies, dim=1)
                            if action_entropies else None)
    ...
    def compute_actor_loss(self, lambda_returns: torch.Tensor, predicted_values_targ: torch.Tensor,
        ...
        if self.actor_gradients == "dynamics":
            ...
            if action_log_probs is None:
                raise ValueError(
                    "actor_gradients='reinforce' needs log pi(a), and the "
                    "attached policy has none. See attach_policy.")
        ...
        if action_entropies is None:
            # A flow policy has no analytic entropy. The bonus is dropped in
            # that mode and nothing is put in its place: a substitute
            # regularizer added quietly here would be a different objective
            # reported under the same name. The return term above is unchanged.
            return {"actor_loss": actor_return_loss,
                    "actor_return_loss": actor_return_loss,
                    "actor_entropy_loss": torch.zeros_like(actor_return_loss)}
    ...
    def select_action(self, observation: torch.Tensor, is_first: bool = False, mode: str = "train") -> torch.Tensor:
        ...
        elif self.latent_policy is not None:
            # The flow policy reads the same causal slot history. It bounds
            # how much of it it reads -- see its `context` -- because the
            # training-time history is bounded and an unbounded one at
            # inference is a different attention profile over the same slots.
            selected_action = self.latent_policy.act(
                self._slot_history, deterministic=(mode == "eval")).squeeze()
        else:
    ...
    def on_save_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        """Keep the latent policy's optimizer and its settings with the weights.

        Lightning already carries the policy's *parameters*: `attach_policy`
        assigns it as a submodule. Its optimizer is not one of the four
        `configure_optimizers` returned, so its state would otherwise be lost
        on a resume, and the settings the weights depend on -- the pretrained
        revision, the chunk length, the action units -- are not recoverable
        from the tensors at all.
        """
        super().on_save_checkpoint(checkpoint)
        if self.latent_policy is not None:
            checkpoint["latent_policy_optimizer"] = self.latent_policy.optimizer.state_dict()
            checkpoint["latent_policy_meta"] = self.latent_policy.descriptor()

    def on_load_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        super().on_load_checkpoint(checkpoint)
        if self.latent_policy is None:
            return
        stored = checkpoint.get("latent_policy_meta")
        if stored is None:
            raise SystemExit(
                "this checkpoint was written without a latent policy and one "
                "is attached. Resume it with attach_policy(None) to take the "
                "world model alone.")
        self.latent_policy.check_descriptor(stored)
        if "latent_policy_optimizer" in checkpoint:
            self.latent_policy.optimizer.load_state_dict(
                checkpoint["latent_policy_optimizer"])
