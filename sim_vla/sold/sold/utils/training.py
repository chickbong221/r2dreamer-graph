...
class OnlineModule(LoggingStepMixin, LightningModule, ABC):
    ...
    def on_load_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        ...
        if replay_buffer_path is not None and replay_buffer_path.exists():
            ...
            # RingBufferDataset spells this `load_from_disk_storage`; there is
            # no `load_from_files`, so resuming a run with a saved replay
            # raised AttributeError from inside on_load_checkpoint -- after
            # Lightning had already restored the weights, so the failure named
            # neither the buffer nor the resume.
            self.replay_buffer.load_from_disk_storage(
                checkpoint["replay_buffer_path"], checkpoint["replay_buffer_capacity"],
