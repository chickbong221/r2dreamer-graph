...
_WANDB_DIAGNOSTICS = {
    ...
    # Progress shaping.
    #   valid_fraction  is the target retained and fully labelled?
    #   target_std      does behaviour produce spread to learn from?
    #   head_mae        has the world model learned the ladder?
    #   critic_mae      has the progress critic caught up? Falling MAE with
    #                   falling influence is the design working.
    #   influence_raw   beta * E|A_progress| / E|A_env|, both unnormalised.
    #                   Not comparable with the old "influence" chart.
    #   horizon_std     the control: a potential flat within a rollout holds
    #                   influence near zero whatever beta does.
    #   shaping_*       must span both signs, or a cycle would pay.
    "train/progress/valid_fraction",
    "train/progress/target_std",
    "train/progress/head_mae",
    "train/progress/critic_mae",
    "train/progress/influence_raw",
    "train/progress/potential_horizon_std",
    "train/progress/shaping_mean",
    "train/progress/shaping_positive_fraction",
    "train/progress_beta",
    # Denominator for every advantage the actor sees.
    "train/actor_scale",
}
