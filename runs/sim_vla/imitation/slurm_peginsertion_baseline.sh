#!/bin/bash
#SBATCH --job-name=r2d-svla-pi-bl-il
...
# sim_vla arm 1 (dreamer): no graph.
# Imitation only: Stage 1A world model -> Stage 1B imitation -> evaluation in
# the simulator. No online stage. Differs from slurm_peginsertion_graph_progress.sh
# only by --experiment. Needs PegInsertionSide-v1/demos.h5 from slurm_collect_data.sh.

echo "================================="
echo "Job started on $(hostname)"
echo "Job ID: $SLURM_JOB_ID"
echo "GPUs allocated: $CUDA_VISIBLE_DEVICES"
echo "sim_vla: PegInsertionSide-v1, arm=dreamer (baseline, no graph), imitation only (1A -> 1B -> eval)"
echo "================================="
...
# Keep in sync with slurm_peginsertion_graph_progress.sh.
WORLD_STEPS=25000
...
OUT_DIR=$HOME/logdir/r2dreamer-graph/sim_vla/$TIMESTAMP/peginsertion/dreamer_imitation_seed${SEED}
echo "[out] $OUT_DIR"
