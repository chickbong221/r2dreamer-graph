#!/bin/bash
#SBATCH --job-name=r2d-svla-sc-bl-il
...
# the simulator. No online stage. Differs from slurm_stackcube_graph_progress.sh
# only by --experiment. Needs StackCube-v1/demos.h5 from slurm_collect_data.sh.

echo "================================="
echo "Job started on $(hostname)"
echo "Job ID: $SLURM_JOB_ID"
echo "GPUs allocated: $CUDA_VISIBLE_DEVICES"
echo "sim_vla: StackCube-v1, arm=dreamer (baseline, no graph), imitation only (1A -> 1B -> eval)"
echo "================================="
...
# Keep in sync with slurm_stackcube_graph_progress.sh.
WORLD_STEPS=25000
...
OUT_DIR=$HOME/logdir/r2dreamer-graph/sim_vla/$TIMESTAMP/stackcube/dreamer_imitation_seed${SEED}
echo "[out] $OUT_DIR"
