#!/bin/bash
#SBATCH --job-name=r2d-svla-pc-bl-il
...
# the simulator. No online stage. Differs from slurm_pickcube_graph_progress.sh
# only by --experiment. Needs PickCube-v1/demos.h5 from slurm_collect_data.sh.

echo "================================="
echo "Job started on $(hostname)"
echo "Job ID: $SLURM_JOB_ID"
echo "GPUs allocated: $CUDA_VISIBLE_DEVICES"
echo "sim_vla: PickCube-v1, arm=dreamer (baseline, no graph), imitation only (1A -> 1B -> eval)"
echo "================================="
...
OUT_DIR=$HOME/logdir/r2dreamer-graph/sim_vla/$TIMESTAMP/pickcube/dreamer_imitation_seed${SEED}
echo "[out] $OUT_DIR"
