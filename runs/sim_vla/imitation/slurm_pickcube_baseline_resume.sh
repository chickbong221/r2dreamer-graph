#!/bin/bash
#SBATCH --job-name=r2d-svla-pc-bl-il-resume
...
# sim_vla arm 1 (dreamer), PickCube: reuses only the world model from
# RESUME_FROM, retrains imitation with slurm_pickcube_baseline.sh's current
...
# from slurm_pickcube_graph_progress_resume.sh only by --experiment and
# RESUME_FROM.
...
echo "sim_vla: PickCube-v1, arm=dreamer (baseline, no graph), world model restored, imitation retrained -> eval"
echo "================================="
...
RESUME_FROM=/home/duongnm2/logdir/r2dreamer-graph/sim_vla/20260923_192116/pickcube/dreamer_imitation_seed0
...
OUT_DIR=$HOME/logdir/r2dreamer-graph/sim_vla/$TIMESTAMP/pickcube/dreamer_imitation_resume_seed${SEED}
mkdir -p "$OUT_DIR"
