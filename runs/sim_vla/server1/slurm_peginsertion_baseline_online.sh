#!/bin/bash
#SBATCH --job-name=r2d-svla-pi-bl-online
...
# Stage 2 only, dreamer (baseline) arm: restore Stage 1A + 1B from an earlier
# --save-checkpoints run and go straight to online training. Everything about
# the arm is unchanged from slurm_peginsertion_baseline.sh -- same partition,
# same resources, same env, same --experiment, same --online-steps -- so this
# stays step-for-step comparable to the graph_progress resume beside it.
#
...
echo "sim_vla: PegInsertionSide-v1, arm=dreamer, stage 2 only (resumed)"
echo "================================="
