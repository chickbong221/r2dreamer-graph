#!/bin/bash
#SBATCH --job-name=r2d-svla-pi-gp
...
# PegInsertionSide-v1.json + subtask_whitelists/PegInsertionSide-v1), and
# beta*(gamma*phi(s') - phi(s)) is added to the imagined advantage during
# stage 2 only -- it is never added to the environment reward that gets
# reported.
#
# Compared against slurm_peginsertion_baseline.sh's "dreamer" arm at the same
...
echo "sim_vla: PegInsertionSide-v1, arm=graph_progress (beta=0.05)"
echo "================================="
...
# Must match slurm_peginsertion_baseline.sh's stage budgets -- the comparison
