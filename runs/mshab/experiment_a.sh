#!/bin/bash
# Experiment A -- five objects, one tidy_house scene, 10M steps.
#
...
python train.py   env=mshab_pick_a   model=size50M_graph_simple   env.graph.whitelist_dir=$WL   model.graph.entity_vocab=$ENTITY_VOCAB   model.graph.n_max=$N_MAX   model.graph.e_max=$E_MAX   checkpoint.enabled=true   checkpoint.metric=$CKPT_METRIC   checkpoint.tiebreak=${CKPT_TIEBREAK:-}   wandb.group=mshab_tidy_house_pick_A   wandb.name=A-five-objects-one-scene   logdir=$HOME/logdir/r2dreamer-graph/$TS/experiment_A
